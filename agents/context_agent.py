...
from utils.response_parser import extract_json_value, normalize_context_output
...
class ContextAgent:
    """Generate structured context candidates for one interaction."""

    def __init__(self) -> None:
        shared = _read_prompt(PROMPTS_DIR / "shared_prompt.txt")
        specific = _read_prompt(PROMPTS_DIR / "context_prompt.txt")
    ...
    def run(
        ...
        prompt = (
            ...
            "{\"contexts\": [{\"label\": \"...\", \"reasoning\": \"...\"}]}"
        )

        response_text = call_model(prompt=prompt, image_paths=image_paths)
        parsed = extract_json_value(response_text)
        return normalize_context_output(parsed if parsed is not None else response_text)
